...
export default function AtividadesPage() {
  ...
  const handleInscricao = (activityId: string) => {
    console.log('Tentando inscrever na atividade:', activityId);
    if (!activityId) {
            ...
            {filteredActivities.length > 0 ? (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                {filteredActivities.map((activity) => {
                  console.log('Renderizando card da atividade:', activity.id, activity.name);
                  return (
                    <div
                      key={activity.id}
                        ...
                        <div className="mt-4 relative z-10">
                          <button
                            ...
                            onClick={(e) => {
                              e.preventDefault();
                              e.stopPropagation();
                              console.log('Botão clicado para atividade:', activity.id, activity.name);
                              handleInscricao(activity.id);
                            }}
                            className="w-full bg-sky-600 text-white py-2 px-4 rounded-lg hover:bg-sky-700 transition-colors font-medium cursor-pointer"
                            style={{ pointerEvents: 'auto' }}
                          >
                            Inscrever-se
                          </button>
